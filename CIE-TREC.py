...
class Itemlist:
    ...
    def return_frequent_items(self, min_supp, min_conf, min_fscore):
        frequent_items = []
        for i in range(0, len(self.items)):
            if(self.items[i].confidence >= min_conf):
...
def main(argv):
    ...
    input_address = 'Train\\TREC-Train.txt'
    ...
    with open(input_address) as f:
        ...
        for line in input_instances:
            ...
            question_class = tokenized_text[0] + tokenized_text[1] + tokenized_text[2]
            question_text = ''
            token_list = []
            for i in range(3, len(tokenized_text)):
                if i != 3:
                    question_text += ' '
                question_text += tokenized_text[i]
                if (i != len(tokenized_text) - 1):
                    current_token = tokenized_text[i]
                    current_token = current_token.lower()
                    if (current_token not in ('the', "'", '``', "''", "`", ",")):
                        token_list.append(current_token)
            print('Class:', question_class)
            print('Question:', question_text)
            print('------------------------------------------------------------------------------------------------')
            
            temp_instance = Instance(question_text, question_class, token_list)
            instances.append(temp_instance)
            
    for i in range(0, len(instances)):
        print('Question:', instances[i].question, '----- Class:', instances[i].label, '----- Tokens:', len(instances[i].question_tokens))
    ...
    output_file = open('Train\\Model-Sup' + str(support_threshold) + '-Conf' + str(confidence_threshold) + '.txt', 'w')
    ...
    output_file = open('Train\\Model-Sup' + str(support_threshold) + '-Conf' + str(confidence_threshold) + '.xml', 'w')
    ...
    input_address = 'Test\\TREC-Test.txt'
    ...
    for i in range(0, len(instances)):
        ...
        for j in range(0, len(instances[i].possible_predictions)):
            ...
            for m in range(0, len(instances[i].possible_predictions[j].itemsets)):
                ...
                result += '\n' + '        Itemset:' + str(instances[i].possible_predictions[j].itemsets[m].items) + '----- F_score:' + str(instances[i].possible_predictions[j].itemsets[m].f_score) + '----- Confidence:' + str(instances[i].possible_predictions[j].itemsets[m].confidence) + '----- Class_support:' + str(instances[i].possible_predictions[j].itemsets[m].class_support) + '----- Support:' + str(instances[i].possible_predictions[j].itemsets[m].support)
